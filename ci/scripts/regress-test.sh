...
buildkite-agent artifact download piestream_regress_test-"$profile" target/debug/
mv target/debug/piestream-"$profile" target/debug/piestream
mv target/debug/risedev-dev-"$profile" target/debug/risedev-dev
mv target/debug/piestream_regress_test-"$profile" target/debug/piestream_regress_test
...
RUST_BACKTRACE=1 target/debug/piestream_regress_test -h db \
  -p 5432 \
  -u postgres \
  --database postgres \
...
echo "--- ci-3cn-1fe, piestream regress test"
