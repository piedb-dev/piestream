...
impl serde::Serialize for TaskOutputId {
        ...
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("task_service.TaskOutputId", len)?;
        if let Some(v) = self.task_id.as_ref() {
            struct_ser.serialize_field("taskId", v)?;
        }
        if self.output_id != 0 {
...
impl<'de> serde::Deserialize<'de> for TaskOutputId {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            ...
            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct task_service.TaskOutputId")
            }
        ...
        deserializer.deserialize_struct("task_service.TaskOutputId", FIELDS, GeneratedVisitor)
    }
}
