...
const SHOW_MATERIALIZED_VIEWS: &str="show materialized views";
...
impl MySQLApi
{
    ...
    pub fn show_materialized_views<'a, W: std::io::Write + Send>(
        &'a self,
        results: QueryResultWriter<'a, W>,
        pg_results: PgResponse
    ) -> Result<()> {
        ...
        let str_col = [Column {
            table: "".to_string(),
            column: "Materialized Views".to_string(),
            coltype: ColumnType::MYSQL_TYPE_STRING,
            colflags: ColumnFlags::empty(),
        }];
        let mut rw = results.start(&str_col)?;
            ...
            match rsp {
                Ok(res) => {
                    match lower_case_sql.as_str() {
                        ...
                        SHOW_MATERIALIZED_VIEWS => self.show_materialized_views(results,res),
                        _ => {
